...
fn render_prompt_modal(f: &mut Frame, area: Rect, prompt: &str, scroll: u16, _syntax: &Syntax) {
    let popup = center_rect(area, 85, 80);
    f.render_widget(Clear, popup);
    // Use tui-markdown (pulldown-cmark + syntect) for proper markdown rendering.
