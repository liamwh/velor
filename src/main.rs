...
fn run_auto_loop(
	...
	while current_iteration <= iterations {
		...
		let prompt_with_context = if !history.is_empty() {
			...
			template::render_template(template_str, &vars)
				.wrap_err_with(|| format!("failed to render template for iteration {current_iteration}"))?
		} else {
			let mut vars = base_vars.clone();
			vars.insert("iteration".to_string(), current_iteration.to_string());
...
fn execute_with_retry(
	...
	for attempt in 1..=config.max_retries {
		...
		if attempt > 1 {
			let delay = retry::calculate_backoff(attempt, config.base_backoff_ms, config.max_backoff_ms);
			println!(
